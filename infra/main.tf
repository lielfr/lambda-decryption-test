...
resource "aws_s3_bucket" "source" {
  bucket = "lielfr-lambdas-test-source-bucket"
}

resource "aws_s3_bucket" "target" {
  bucket = "lielfr-lambdas-test-target-bucket"
}

resource "aws_secretsmanager_secret" "private-key" {
  name = "lambdas-test-private-key"
}
...
resource "aws_lambda_function" "s3-lambda" {
  ...
  function_name    = "decryption-lambda-test"
  role             = aws_iam_role.iam_for_lambda.arn
  handler          = "bootstrap.App"
  architectures    = ["arm64"]
  source_code_hash = filesha512("../lambda.zip")
  environment {
    variables = {
...
resource "aws_iam_policy_attachment" "lambda-role-policy-basic-execution-role" {
  ...
  name       = "role-policy-attachment-lambda-decryption-basic-execution"
}
